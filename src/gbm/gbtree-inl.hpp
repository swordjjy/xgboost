...
#pragma omp parallel
      ...
      virtual void Serialize(flatbuffers::FlatBufferBuilder& fbb) const {
        ...
        for (int i = 0; i < trees.size(); ++i) {
          const tree::RegTree* tree = trees[i];
          _trees.emplace_back(tree->Serialize(fbb));
        }
      ...
      struct ModelParam {
        ...
        */
        ...
        std::string toString() const{
          ...
          std::string params = "num_trees: "+ std::to_string((_Longlong)num_trees)+"num_roots: "+std::to_string((_Longlong)num_roots)
            +"num_pbuffer: "+std::to_string(num_pbuffer)+"num_pbuffer: "+std::to_string(num_pbuffer)
            +"num_output_group: "+std::to_string((_Longlong)num_output_group)
            +"size_leaf_vector: "+std::to_string((_Longlong)size_leaf_vector);
